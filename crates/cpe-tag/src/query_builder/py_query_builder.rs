...
pub fn get_grep_patterns(validated_json: &str) -> Result<String, Box<dyn Error>> {
    let from_python = call_python_code(validated_json);
    Ok(from_python?.to_string())
}

fn call_python_code(validated_json: &str) -> PyResult<Py<PyAny>> {
    ...
    Python::with_gil(|py| -> PyResult<Py<PyAny>> {
        ...
        integrator.call1(py, (validated_json,))
    })
}
